...
if (~isempty(varargin))
   ...
   vnAITensorSize = [varargin{:}];
   
   % - Take Matlab semantics to make square matrices
   if (isscalar(vnAITensorSize))
      vnAITensorSize = vnAITensorSize([1 1]);
   end
...
elseif (~any(vbScalarArgs))
   % - Two non-scalar arguments
   if (~isequal(size(fMu), size(fKappa)))
      error('VMRAND:UnequalSizes', ...
         '*** vmrand: ''fMu'' and ''fKappa must be the same dimensions.');
   else
      vnTensorSize = size(fMu);
   end
      
else
   % - Use explicit size arguments
   vnTensorSize = vnAITensorSize;
end

% - Check that fMu and fKappa supplied tensor sizes match explicit sizes
if (prod(vnAITensorSize) > 1) && ~isequal(vnTensorSize, vnAITensorSize)
   error('VMRAND:UnequalSizes', ...
         '*** vmrand: Non-scalar ''fMu'' and ''fKappa'' do not match explicitly provided tensor size [M, N, P, ...].');
end
...
tfVMVariates = nan(1, prod(vnTensorSize));
tfZ = nan(1, prod(vnTensorSize));
tfF = nan(1, prod(vnTensorSize));
tfC = nan(1, prod(vnTensorSize));


% -- Re-shape inputs

fKappa = reshape(fKappa, 1, []);
fMu = reshape(fMu, 1, []);
...
% -- Re-shape output
tfVMVariates = reshape(tfVMVariates, vnTensorSize);
